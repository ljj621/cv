...
def get_indice_pairs(indices,
    ...
    if grid is None:
        if ndim == 2:
            get_indice_pairs_func = spconv_ext.get_indice_pairs_2d
        elif ndim == 3:
            get_indice_pairs_func = spconv_ext.get_indice_pairs_3d
        elif ndim == 4:
            get_indice_pairs_func = spconv_ext.get_indice_pairs_4d
        else:
        ...
        if ndim == 2:
            get_indice_pairs_func = spconv_ext.get_indice_pairs_grid_2d
        elif ndim == 3:
            get_indice_pairs_func = spconv_ext.get_indice_pairs_grid_3d
        else:
            raise NotImplementedError
...
def indice_conv(features,
    ...
    if filters.dtype == torch.float32:
        return spconv_ext.indice_conv_fp32(features, filters,
                                                indice_pairs, indice_pair_num,
                                                num_activate_out, int(inverse),
                                                int(subm))
    elif filters.dtype == torch.half:
        return spconv_ext.indice_conv_half(features, filters,
                                                indice_pairs, indice_pair_num,
                                                num_activate_out, int(inverse),
                                                int(subm))
    else:
...
def fused_indice_conv(features, filters, bias, indice_pairs, indice_pair_num,
    ...
    if features.dtype == torch.half:
        func = spconv_ext.fused_indice_conv_half
    elif filters.dtype == torch.float32:
        func = spconv_ext.fused_indice_conv_fp32
    else:
        raise NotImplementedError
...
def indice_conv_backward(features,
                         ...
                         subm=False):
    if filters.dtype == torch.float32:
        return spconv_ext.indice_conv_backward_fp32(
            features, filters, out_bp, indice_pairs, indice_pair_num,
            int(inverse), int(subm))
    elif filters.dtype == torch.half:
        return spconv_ext.indice_conv_backward_half(
            features, filters, out_bp, indice_pairs, indice_pair_num,
            int(inverse), int(subm))
    else:
        raise NotImplementedError
...
def indice_maxpool(features, indice_pairs, indice_pair_num, num_activate_out):
    if features.dtype == torch.float32:
        return spconv_ext.indice_maxpool_fp32(features, indice_pairs,
                                                   indice_pair_num,
                                                   num_activate_out)
    elif features.dtype == torch.half:
        return spconv_ext.indice_maxpool_half(features, indice_pairs,
                                                   indice_pair_num,
                                                   num_activate_out)
    else:
        raise NotImplementedError
...
def indice_maxpool_backward(features, out_features, out_bp, indice_pairs,
                            indice_pair_num):
    if features.dtype == torch.float32:
        return spconv_ext.indice_maxpool_backward_fp32(
            features, out_features, out_bp, indice_pairs, indice_pair_num)
    elif features.dtype == torch.half:
        return spconv_ext.indice_maxpool_backward_half(
            features, out_features, out_bp, indice_pairs, indice_pair_num)
    else:
        raise NotImplementedError
